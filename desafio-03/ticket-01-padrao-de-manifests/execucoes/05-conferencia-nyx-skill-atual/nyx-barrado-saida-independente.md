...
- KSV-0004 (LOW): Default capabilities: some containers do not drop any
  - nyx-barrado.yaml: Container 'api' of 'deployment' 'NyxAPI' in 'nyx-prod' namespace should set securityContext.capabilities.drop
- KSV-0021 (LOW): Runs with GID <= 10000
  - nyx-barrado.yaml: Container 'api' of Deployment 'NyxAPI' should set 'securityContext.runAsGroup' > 10000
- KSV-0030 (LOW): Runtime/Default Seccomp profile not set
  - nyx-barrado.yaml: Either Pod or Container should set 'securityContext.seccompProfile.type' to 'RuntimeDefault'
- KSV-0104 (MEDIUM): Seccomp policies disabled
  - nyx-barrado.yaml: container "api" of deployment "NyxAPI" in "nyx-prod" namespace should specify a seccomp profile
- KSV-0106 (LOW): Container capabilities must only include NET_BIND_SERVICE
  - nyx-barrado.yaml: container should drop all
- KSV-0118 (HIGH): Default security context configured
  - nyx-barrado.yaml: container NyxAPI in nyx-prod namespace is using the default security context
  - nyx-barrado.yaml: deployment NyxAPI in nyx-prod namespace is using the default security context, which allows root privileges
